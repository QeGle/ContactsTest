...
class ListContactVM(
    ...
    fun get() {
        state.postValue(LoadingState)
        disposable += contactInteractor.getContacts(DataFetchStrategy.PreferLocal)
            .map { it.toContactUIList() }
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
    ...
    fun update() {
        ...
        disposable += contactInteractor.getContacts(DataFetchStrategy.Remote)
            .map { it.toContactUIList() }
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(getDisposableSingleObserver())
    ...
    private fun getDisposableSingleObserver() =
        object : DisposableSingleObserver<List<ContactUI>?>() {
